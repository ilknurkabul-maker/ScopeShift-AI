...
- DEPENDENCY: feature mentions a dependency not present.`;
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: { parts: [{ text: prompt }] },
            config: {
                systemInstruction: "You are a senior product manager AI that analyzes product scopes for issues based on a set of rules. You must follow the user's instructions precisely and only return JSON that adheres to the provided schema.",
                responseMimeType: "application/json",
...
`;
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: { parts: [{ text: prompt }] },
            config: {
                systemInstruction: "You are an AI test engineer that creates API test plans from product specifications. You must follow the user's instructions precisely and only return JSON that adheres to the provided schema.",
                responseMimeType: "application/json",
...
`;
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: { parts: [{ text: prompt }] },
            config: {
                systemInstruction: "You are an AI product manager that proposes scope changes to meet new constraints. You must follow the user's instructions precisely and only return JSON that adheres to the provided schema.",
                responseMimeType: "application/json",
...
<code>`;
    try {
        ...
        console.error("Error finalizing scope with Gemini API:", error);
        if (error instanceof Error) {
            throw new Error(`Failed to finalize scope: ${error.message}`);
        }
        throw new Error("An unknown error occurred while finalizing scope.");
    }
};